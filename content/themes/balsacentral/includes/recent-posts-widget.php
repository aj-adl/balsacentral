...
class balsacentral_Recent_Posts extends WP_Widget {
	function balsacentral_Recent_Posts() {
		$widget_ops = array( 'classname' => 'balsacentral_recent_entries', 'description' => __( 'List recent posts with thumbnails from custom categories.', 'balsacentral' ) );
		$this->WP_Widget('balsacentral-recent-posts', __( 'balsacentral Recent Posts', 'balsacentral' ), $widget_ops);
		$this->alt_option_name = 'balsacentral_recent_entries';
	...
	function widget($args, $instance) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __( 'Recent Posts', 'balsacentral' ) : $instance['title']);
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['balsacentral_recent_entries']) )
		delete_option('balsacentral_recent_entries');
		return $instance;
	}
	...
	function form( $instance ) {
		...
		if ( !isset($instance['number']) || !$number = (int) $instance['number'] )
			$number = 5; ?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title:', 'balsacentral' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
        </p>
		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e( 'Number of posts to show:', 'balsacentral' ); ?></label>
		<input id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" size="3" /><br />
		<small><?php _e( '(at most 15)', 'balsacentral' ); ?></small>
        </p>
		<p><label for="<?php echo $this->get_field_id('offset'); ?>"><?php _e( 'Posts offset', 'balsacentral' ); ?></label>
		...
		<small><?php _e( 'Provide an offset number to which you wish to skip the posts.', 'balsacentral' ); ?></small>
        </p>
		<p><label for="<?php echo $this->get_field_id('cats'); ?>"><?php _e( 'Cat IDs to exclude or include:', 'balsacentral' ); ?></label>
		...
		<br />
		<small><?php _e( 'Category IDs, separated by commas. Eg: 3,6,7 to include. Or -3,-6,-7 to exclude.', 'balsacentral' ); ?></small>
		</p>
        <p><label for="<?php echo $this->get_field_id( 'hide_thumb' ); ?>"><?php _e( 'Hide Thumbnails?', 'balsacentral' ); ?></label>
        ...
        <small><?php _e( 'If unchecked, it will show post thumbnails.', 'balsacentral' ); ?></small>
        </p>
	<?php }
}?>