...
<div id="content"<?php if ( $bcent_sb_pos == 'left' ) echo (' class="content-right"'); ?> role="main">
    ...
    $count = count($cat_ids);
    if ( $count > 0 ){ ?>
        <ul id="filter-nav"><li class="filter-nav-label"><?php _e( 'Filter by', 'balsacentral' ); ?></li><li class="current all"><a href="#"><?php _e( 'All', 'balsacentral' ); ?></a></li>
			...
			<?php while ($wp_query->have_posts()) : $wp_query->the_post();
					...
					<li class="port-item <?php echo strtolower($tax); ?>" data-id="<?php echo 'port-id-'.$count; ?>" style="z-index:<?php echo $count; ?>">
                        <div class="port-details">
                            ...
                            <a class="image-link" data-rel="prettyPhoto[group1]" href="<?php echo $big; ?>"><?php _e( 'View Image', 'balsacentral' ); ?></a><a class="perma-link" href="<?php echo $permalink; ?>"><?php _e( 'Permalink', 'balsacentral' ); ?></a>
                            </span><!-- .port-meta -->
                        </div><!-- .port-details -->
