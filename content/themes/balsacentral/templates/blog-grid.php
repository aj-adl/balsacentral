...
get_header(); ?>
<div id="content-grid"<?php if ( $bcent_sb_pos == 'left' ) echo (' class="content-right"'); ?> role="main">
    ...
    if ($category) {
		...
		<div id="mason_container" class="clearfix">
			<?php while ($wp_query->have_posts()) : $wp_query->the_post(); ?>
                <article id="post-<?php the_ID();?>" <?php post_class('entry-grid'); ?>>
                    ...
                    <aside id="meta-<?php the_ID();?>" class="entry-meta"><?php balsacentral_small_meta(); ?></aside>
                    <?php } ?>
                    </div><!-- .entry-content -->
		...
		if ( $wp_query->max_num_pages > 1 ) :?>
            ...
            <h2 class="entry-title"><?php _e( 'Not Found', 'balsacentral' ); ?></h2>
