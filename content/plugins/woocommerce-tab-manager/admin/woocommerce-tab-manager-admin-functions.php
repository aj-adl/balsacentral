							...
							<tr>
								<td>
									...
									<?php if ( 'third_party' == $tab['type'] ) : ?>
										<p><em><?php echo esc_html( $tab['description'] ? $tab['description'] : __( 'The title/content for this tab will be provided by a third party plugin', WC_Tab_Manager::TEXT_DOMAIN ) ); ?></em></p>
									<?php endif; ?>
									<div class="options_group">
										<?php if ( 'third_party' != $tab['type'] ) : ?>
											<p class="form-field product_tab_title_field">
												...
												<?php else: ?>
													<input type="text" value="<?php echo esc_attr( $tab['title'] ); ?>" id="product_tab_title_<?php echo $i; ?>" name="product_tab_title[<?php echo $i; ?>]" class="short product_tab_title"> <span class="description"><?php esc_html_e( "The tab title, this appears in the tab", WC_Tab_Manager::TEXT_DOMAIN ); ?></span>
												<?php endif; ?>
											</p>
										<?php endif; ?>
										<?php if ( isset( $core_tabs[ $id ]['heading'] ) && $core_tabs[ $id ]['heading'] ) : ?>
											<p class="form-field product_tab_heading_field">
												...
												<input type="text" value="<?php echo esc_attr( $tab['heading'] ); ?>" id="product_tab_heading_<?php echo $i; ?>" name="product_tab_heading[<?php echo $i; ?>]" class="short"> <span class="description"><?php esc_html_e( "The tab heading, this appears just before the tab content", WC_Tab_Manager::TEXT_DOMAIN ); ?></span>
											</p>
										<?php endif; ?>
										<?php if ( 'global' == $tab['type'] ) : ?>
											<p class="form-field product_tab_heading_field">
...
/**
 * Removes the WPSEO premium slug change filter, if set, so that
 * bogus redirects don't get created
 *
 * @since 1.1.2
 * @param string $filter_name the filter name to remove, ie 'post_updated' or 'delete_post'
 * @param string $callback_name the callback method name, ie 'detect_slug_change' or 'detect_post_delete'
 * @return array containing the filter name, priority, key, and filter function, if set
 */
function remove_wpseo_premium_filter( $filter_name, $callback_name ) {

	// stop Yoast SEO Premium from creating redirects
	if ( class_exists( 'WPSEO_Premium' ) ) {
		global $wp_filter;

		foreach ( $wp_filter[ $filter_name ] as $priority => $filters ) {
			foreach ( $filters as $key => $filter ) {
				if ( false !== strpos( $key, $callback_name ) ) {
					unset( $wp_filter[ $filter_name ][ $priority ][ $key ] );
					return array( $filter_name, $priority, $key, $filter );
				}
			}
		}
	}
...
/**
 * Restores the given WPSEO premium filter, if set
 *
 * @since 1.1.2
 * @param $data array containing the filter name, priority, key, and filter function, if set
 */
function restore_wpseo_premium_filter( $data ) {
	...
	global $wp_filter;
	$wp_filter[ $data[0] ][ $data[1] ][ $data[2] ] = $data[3];
}
